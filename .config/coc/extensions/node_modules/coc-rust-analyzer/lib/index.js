    ...
    Utils.prototype.findFiles = function(path2) {
      ...
      function findSync(dir, pattern, recursive) {
        ...
        self.fs.readdirSync(dir).forEach(function(file) {
          ...
          if (self.fs.statSync(path3).isDirectory() && recursive) files = files.concat(findSync(path3, pattern, recursive));
...
var analyzerStatus = new lc.RequestType(
  "rust-analyzer/analyzerStatus"
);
var cancelFlycheck = new lc.NotificationType0("rust-analyzer/cancelFlycheck");
var clearFlycheck = new lc.NotificationType0("rust-analyzer/clearFlycheck");
var expandMacro = new lc.RequestType(
  "rust-analyzer/expandMacro"
);
var memoryUsage = new lc.RequestType0("rust-analyzer/memoryUsage");
var openServerLogs = new lc.NotificationType0("rust-analyzer/openServerLogs");
var relatedTests = new lc.RequestType(
  "rust-analyzer/relatedTests"
);
var reloadWorkspace = new lc.RequestType0("rust-analyzer/reloadWorkspace");
var rebuildProcMacros = new lc.RequestType0("rust-analyzer/rebuildProcMacros");
var runFlycheck = new lc.NotificationType("rust-analyzer/runFlycheck");
var shuffleCrateGraph = new lc.RequestType0("rust-analyzer/shuffleCrateGraph");
var syntaxTree = new lc.RequestType(
  "rust-analyzer/syntaxTree"
);
var viewCrateGraph = new lc.RequestType(
  "rust-analyzer/viewCrateGraph"
);
var viewFileText = new lc.RequestType(
  "rust-analyzer/viewFileText"
);
var viewHir = new lc.RequestType(
  "rust-analyzer/viewHir"
);
var viewMir = new lc.RequestType(
  "rust-analyzer/viewMir"
);
var interpretFunction = new lc.RequestType(
  "rust-analyzer/interpretFunction"
);
var viewItemTree = new lc.RequestType(
  "rust-analyzer/viewItemTree"
);
var discoverTest = new lc.RequestType(
  "experimental/discoverTest"
);
var discoveredTests = new lc.NotificationType(
  "experimental/discoveredTests"
);
var runTest = new lc.RequestType("experimental/runTest");
var abortRunTest = new lc.NotificationType0("experimental/abortRunTest");
var endRunTest = new lc.NotificationType0("experimental/endRunTest");
var appendOutputToRunTest = new lc.NotificationType(
  "experimental/appendOutputToRunTest"
);
var changeTestState = new lc.NotificationType(
  "experimental/changeTestState"
);
var fetchDependencyList = new lc.RequestType("rust-analyzer/fetchDependencyList");
var fetchDependencyGraph = new lc.RequestType("rust-analyzer/fetchDependencyGraph");
var joinLines = new lc.RequestType(
  "experimental/joinLines"
);
var matchingBrace = new lc.RequestType(
  "experimental/matchingBrace"
);
var moveItem = new lc.RequestType(
  "experimental/moveItem"
);
var onEnter = new lc.RequestType(
  "experimental/onEnter"
);
var openCargoToml = new lc.RequestType(
  "experimental/openCargoToml"
);
var openDocs = new lc.RequestType(
  "experimental/externalDocs"
);
var parentModule = new lc.RequestType("experimental/parentModule");
var runnables = new lc.RequestType(
  "experimental/runnables"
);
var serverStatus = new lc.NotificationType(
  "experimental/serverStatus"
);
var ssr = new lc.RequestType("experimental/ssr");
var viewRecursiveMemoryLayout = new lc.RequestType("rust-analyzer/viewRecursiveMemoryLayout");
var unindexedProject = new lc.NotificationType(
...
var ExperimentalFeatures = class {
  fillClientCapabilities(capabilities) {
    const caps = capabilities.experimental ?? {};
...
function createClient(bin, config) {
  ...
  function standalone(root) {
    if (!root) return true;
    if ((0, import_node_fs.existsSync)((0, import_node_path.join)(import_coc.Uri.parse(root).fsPath, "Cargo.toml"))) return false;
    if ((0, import_node_fs.existsSync)((0, import_node_path.join)(import_coc.Uri.parse(root).fsPath, "rust-project.json"))) return false;
    if ((0, import_node_fs.existsSync)((0, import_node_path.join)(import_coc.Uri.parse(root).fsPath, ".rust-project.json"))) return false;
    return true;
  }
  let initializationOptions = import_coc.workspace.getConfiguration("rust-analyzer");
  if (import_coc.workspace.workspaceFolders.every((folder2) => standalone(folder2.uri))) {
    ...
    middleware: {
      ...
      async provideCodeActions(document2, range, context, token) {
        ...
        for (const item of values) {
          ...
          const action = import_vscode_languageserver_protocol.CodeAction.create(item.title, command, kind);
          action.edit = {};
          result.push(action);
        }
        return result;
...
var Config = class {
  constructor() {
    ...
    this.requiresReloadOpts = ["server", "cargo", "procMacro", "files", "updates", "lens", "inlayHints"].map(
  ...
  async onConfigChange(event) {
    ...
    let reload2 = !!this.restartServerOnConfigChange;
    if (!reload2) {
      const msg = `Changing "${requiresReloadOpt}" requires a reload`;
      reload2 = await import_coc2.window.showPrompt(`${msg}. Reload now?`);
    }
    if (reload2) {
  ...
  get serverPath() {
    return this.cfg.get("server.path") ?? this.cfg.get("serverPath");
  }
  get serverExtraEnv() {
    return this.cfg.get("server.extraEnv") ?? {};
  }
  ...
  get inlayHint() {
    return {
      enable: import_coc2.workspace.getConfiguration("inlayHint").get("enable", true)
    };
  }
  get debug() {
    return {
      ...
      nvimdapConfiguration: {
        template: this.cfg.get("debug.nvimdap.configuration.template")
      }
    };
  }
  get prompt() {
...
var pipeline = import_node_util.default.promisify(import_node_stream.default.pipeline);
var rejectUnauthorized = import_coc3.workspace.getConfiguration("http").get("proxyStrictSSL", true);
var proxy = process.env.https_proxy || process.env.HTTPS_PROXY;
var agent = proxy ? new import_https_proxy_agent.HttpsProxyAgent(proxy, { rejectUnauthorized }) : null;
...
`;
  ...
  await import_node_fs2.promises.rename(dest, origFile);
  await new Promise((resolve, reject) => {
    var _a, _b;
    const handle = (0, import_node_child_process.exec)(`nix-build -E - --arg src '${origFile}' -o ${dest}`, (err, stdout, stderr) => {
      if (err != null) {
        reject(Error(stderr));
      } else {
        resolve(stdout);
  ...
  await import_node_fs2.promises.unlink(origFile);
}
function isMusl() {
  const res = (0, import_node_child_process.spawnSync)("ldd", ["--version"]);
...
async function downloadServer(context, release) {
  ...
  const _path = import_node_path2.default.join(context.storagePath, release.name);
  const randomHex = (0, import_node_crypto.randomBytes)(5).toString("hex");
  const tempFile = import_node_path2.default.join(context.storagePath, `${release.name}${randomHex}`);
  if (process.platform === "win32") {
    await import_node_fs2.promises.writeFile(tempFile, await resp.buffer());
    new import_adm_zip.default(tempFile).extractAllTo(context.storagePath, true, true);
    await import_node_fs2.promises.unlink(tempFile).catch((err) => {
      console.error(err);
    });
    statusItem.hide();
    return;
  }
  const destFileStream = (0, import_node_fs2.createWriteStream)(tempFile, { mode: 493 });
  ...
  await import_node_fs2.promises.unlink(_path).catch((err) => {
    if (err.code !== "ENOENT") throw err;
  });
  await import_node_fs2.promises.rename(tempFile, _path);
  await context.globalState.update("release", release.tag);
  try {
    if (await import_node_fs2.promises.stat("/etc/nixos")) {
...
function isCargoTomlDocument(document2) {
  const u = import_coc4.Uri.parse(document2.uri);
...
var Ctx = class {
  constructor(extCtx) {
    ...
    this.usingSystemServer = false;
    const statusBar = import_coc4.window.createStatusBarItem(0);
    statusBar.text = "rust-analyzer";
    statusBar.show();
    this.extCtx.subscriptions.push(statusBar);
    import_coc4.window.onDidChangeActiveTextEditor((editor) => {
  ...
  async startServer() {
    ...
    const watcher = import_coc4.workspace.createFileSystemWatcher("**/Cargo.toml");
    this.extCtx.subscriptions.push(watcher);
    watcher.onDidChange(async () => await import_coc4.commands.executeCommand("rust-analyzer.reloadWorkspace"));
    ...
    client.onNotification(serverStatus, async (status) => {
      ...
      if (status.health !== "ok" && ((_a = status.message) == null ? void 0 : _a.length)) {
        ...
        import_coc4.window.showNotification({ content: status.message });
  ...
  resolveBin() {
    ...
    let bin = (0, import_node_path3.join)(this.extCtx.storagePath, executableName);
    if (this.config.serverPath) {
      bin = import_which.default.sync(import_coc4.workspace.expand(this.config.serverPath), { nothrow: true }) || bin;
    }
    if ((0, import_node_fs3.existsSync)(bin)) {
      return bin;
    }
    const systemBin = import_which.default.sync(executableName, { nothrow: true });
    if (systemBin) {
      const { stderr } = (0, import_node_child_process2.spawnSync)(systemBin, ["--version"], { encoding: "utf8" });
  ...
  async installServerFromGitHub() {
    ...
    try {
      ...
      import_coc4.window.showInformationMessage(msg, "error");
      return;
    }
    await this.client.stop();
    this.client.start();
  ...
  async checkUpdate(auto = true) {
    ...
    if (old === latest.tag) {
      if (!auto) {
        import_coc4.window.showInformationMessage("Your Rust Analyzer release is updated");
      }
      return;
    }
    ...
    if (this.config.prompt === true) {
      ret = await import_coc4.window.showQuickpick(
    ...
    if (ret === 0) {
      ...
      try {
        ...
        import_coc4.window.showInformationMessage(msg2, "error");
        return;
      }
      await this.client.stop();
      this.client.start();
      ...
      await import_coc4.commands.executeCommand("vscode.open", "https://github.com/rust-analyzer/rust-analyzer/releases").catch(() => {
      });
    }
  }
};
...
function reload(ctx) {
  return async () => {
    import_coc5.window.showInformationMessage("Reloading rust-analyzer...");
    await ctx.client.stop();
    await ctx.client.start();
    import_coc5.window.showInformationMessage("Reloaded rust-analyzer");
  };
}
function analyzerStatus2(ctx) {
  return async () => {
    const { document: document2 } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document2)) return;
    const params = {
      textDocument: { uri: document2.uri }
    };
    const ret = await ctx.client.sendRequest(analyzerStatus, params);
    import_coc5.window.echoLines(ret.split("\n"));
  };
}
...
function matchingBrace2(ctx) {
  return async () => {
    const { document: document2, position } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document2)) return;
    const params = {
      textDocument: { uri: document2.uri },
      positions: [position]
    ...
    if (response.length > 0) {
      import_coc5.workspace.jumpTo(document2.uri, response[0]);
    }
  };
}
function joinLines2(ctx) {
  return async () => {
    ...
    let range = null;
    const mode = await import_coc5.workspace.nvim.call("visualmode");
    if (mode) range = await import_coc5.window.getSelectedRange(mode);
    if (!range) {
      const state = await import_coc5.workspace.getCurrentState();
      range = import_coc5.Range.create(state.position, state.position);
    }
    const param = {
...
function parentModule2(ctx) {
  return async () => {
    var _a, _b;
    const { document: document2, position } = await import_coc5.workspace.getCurrentState();
    if (!(isRustDocument(document2) || isCargoTomlDocument(document2))) return;
    const param = {
      textDocument: { uri: document2.uri },
      position
    };
    ...
    if (locations.length === 1) {
      ...
      const uri = import_coc5.Location.is(loc) ? loc.uri : loc.targetUri;
      const pos = import_coc5.Location.is(loc) ? (_a = loc.range) == null ? void 0 : _a.start : (_b = loc.targetRange) == null ? void 0 : _b.start;
      import_coc5.workspace.jumpTo(uri, pos);
    } else {
      const uri = document2.uri;
      const refs = [];
      for (const l of locations) {
        refs.push(import_coc5.Location.is(l) ? l : import_coc5.Location.create(l.targetUri, l.targetRange));
      }
      await import_coc5.commands.executeCommand("editor.action.showReferences", import_coc5.Uri.parse(uri), position, refs);
    }
  };
}
function ssr2(ctx) {
  return async () => {
    const input = await import_coc5.workspace.callAsync("input", ["Enter request like: foo($a, $b) ==>> ($a).foo($b): "]);
    import_coc5.workspace.nvim.command("normal! :<C-u>", true);
    if (!input) {
      return;
    }
    if (!input.includes("==>>")) {
    ...
    const mode = await import_coc5.workspace.nvim.call("visualmode");
    if (mode) {
      const range = await import_coc5.window.getSelectedRange(mode);
    ...
    import_coc5.window.withProgress({ title: "Structured search replacing...", cancellable: false }, async () => {
      const edit = await ctx.client.sendRequest(ssr, param);
      await import_coc5.workspace.applyEdit(edit);
    });
  };
}
function serverVersion(ctx) {
  return async () => {
    ...
    if (!bin) {
      ...
      import_coc5.window.showErrorMessage(msg);
      return;
    }
    const version = (0, import_node_child_process3.spawnSync)(bin, ["--version"], { encoding: "utf-8" }).stdout.toString();
    import_coc5.window.showInformationMessage(version);
  };
}
async function fetchRunnable(ctx) {
  const { document: document2, position } = await import_coc5.workspace.getCurrentState();
  if (!isRustDocument(document2)) return [];
  import_coc5.window.showInformationMessage("Fetching runnable...");
  const params = {
    textDocument: { uri: document2.uri },
    position
  };
  return await ctx.client.sendRequest(runnables, params);
}
async function pickRunnable(ctx) {
  const runnables2 = await fetchRunnable(ctx);
  if (!runnables2.length) return;
  const items = [];
  for (const r of runnables2) {
    items.push(new RunnableQuickPick(r));
  }
  const idx = await import_coc5.window.showQuickpick(items.map((o) => o.label));
  if (idx === -1) return;
...
function testCurrent(ctx) {
  return async () => {
    const runnables2 = await fetchRunnable(ctx);
    if (!runnables2.length) {
      import_coc5.window.showInformationMessage("No runnables found");
      return;
    }
    const testRunnable = runnables2.find((run2) => run2.label.startsWith("cargo test"));
...
function debugSingle(ctx) {
  return async (runnable) => {
    const { document: document2 } = await import_coc5.workspace.getCurrentState();
    if (!runnable || !isRustDocument(document2)) return;
    let args = [];
    ...
    const proc = (0, import_node_child_process3.spawn)(runnable.kind, args, { shell: true });
    const stderr_rl = import_node_readline.default.createInterface({
      input: proc.stderr,
      crlfDelay: Infinity
    });
    import_coc5.window.withProgress({ title: "Building Debug Target", cancellable: false }, async (progress) => {
    ...
    for await (const line of rl) {
      ...
      if (expectedKind !== void 0 && !cargoMessage["target"]["kind"].includes(expectedKind)) {
        console.debug(`Wrong kind: ${cargoMessage["target"]["kind"]}, expected ${expectedKind}`);
        continue;
      }
      if (expectedName !== void 0 && cargoMessage["target"]["name"] !== expectedName) {
    ...
    if (runtime === "termdebug") {
      await import_coc5.workspace.nvim.command(`TermdebugCommand ${executable} ${executableArgs}`);
      return;
    }
    if (runtime === "vimspector") {
      ...
      await import_coc5.workspace.nvim.call("vimspector#LaunchWithSettings", configuration);
      return;
    }
    if (runtime === "nvim-dap") {
      const template = ctx.config.debug.nvimdapConfiguration.template;
      const args2 = executableArgs.split(" ").filter((s) => s !== "").map((s) => `"${s}"`).join(",");
      const configuration = template == null ? void 0 : template.replace("$exe", `"${executable}"`).replace("$args", `{${args2}}`);
      await import_coc5.workspace.nvim.lua(`require("dap").run(${configuration})`);
      return;
    }
...
function runSingle(ctx) {
  return async (runnable) => {
    ...
    terminal = await import_coc5.window.createTerminal(opt);
    terminal.sendText(cmd);
    if (ctx.config.terminal.startinsert) {
      await import_coc5.workspace.nvim.command("startinsert");
    }
  };
}
function syntaxTree2(ctx) {
  return async () => {
    ...
    if (!ret) return;
    const nvim2 = import_coc5.workspace.nvim;
    nvim2.pauseNotification();
    nvim2.command("edit +setl\\ buftype=nofile [SyntaxTree]", true);
    nvim2.command("setl nobuflisted bufhidden=wipe", true);
    nvim2.call("append", [0, ret.split("\n")], true);
    nvim2.command("exe 1", true);
    await nvim2.resumeNotification(true);
  };
}
function expandMacro2(ctx) {
  return async () => {
    ...
    const nvim2 = import_coc5.workspace.nvim;
    nvim2.pauseNotification();
    nvim2.command("edit +setl\\ buftype=nofile [Macro]", true);
    nvim2.command("setl nobuflisted bufhidden=wipe", true);
    nvim2.command("setl filetype=rust", true);
    nvim2.call("append", [0, lines], true);
    nvim2.command("exe 1", true);
    await nvim2.resumeNotification(true);
  };
}
function explainError(ctx) {
  return async () => {
    ...
    const { document: document2, position } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document2)) return;
    const diag = (_b = (_a = ctx.client.diagnostics) == null ? void 0 : _a.get(document2.uri)) == null ? void 0 : _b.find((diagnostic) => isInRange(diagnostic.range, position));
    if (diag == null ? void 0 : diag.code) {
      const explanation = (0, import_node_child_process3.spawnSync)("rustc", ["--explain", `${diag.code}`], { encoding: "utf-8" }).stdout.toString();
      const docs = [];
      let isCode = false;
      ...
      const float = import_coc5.window.createFloatFactory({});
...
function showReferences() {
  return async (uri, position, locations) => {
    ...
    await import_coc5.commands.executeCommand("editor.action.showReferences", import_coc5.Uri.parse(uri), position, locations);
  };
}
  ...
  if (import_vscode_languageserver_protocol2.TextDocumentEdit.is(change)) {
    ...
    for (const indel of change.edits) {
      ...
      if (index0 !== -1) {
        ...
        position = import_coc5.Position.create(line, col);
      }
      newEdits.push(import_coc5.TextEdit.replace(range, parsed.replaceAll("$0", "")));
    }
    const current = await import_coc5.workspace.document;
    if (current.uri !== change.textDocument.uri) {
      await import_coc5.workspace.loadFile(change.textDocument.uri);
      await import_coc5.workspace.jumpTo(change.textDocument.uri);
    }
    await import_coc5.workspace.applyEdit({ changes: { [change.textDocument.uri]: newEdits } });
    if (position) {
      await import_coc5.window.moveTo(position);
    }
  }
}
...
function runFlycheck2(ctx) {
  return async () => {
    const { document: document2 } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document2)) return;
    ctx.client.sendNotification(runFlycheck, { textDocument: { uri: document2.uri } });
  };
}
function cancelFlycheck2(ctx) {
  return async () => {
    ctx.client.sendNotification(cancelFlycheck);
  };
}
function clearFlycheck2(ctx) {
  return async () => {
    ctx.client.sendNotification(clearFlycheck);
  };
}
...
function openDocs2(ctx) {
  return async () => {
    ...
    if (doclink) {
      if (doclink.local) {
        const exist = (0, import_node_fs4.existsSync)(import_coc5.Uri.parse(doclink.local).fsPath);
        if (exist) {
          await import_coc5.nvim.call("coc#ui#open_url", doclink.local);
          return;
        }
      }
      if (doclink.web) {
        await import_coc5.commands.executeCommand("vscode.open", import_coc5.Uri.parse(doclink.web));
      }
    }
  };
}
function openCargoToml2(ctx) {
  return async () => {
    ...
    if (!location) return;
    await import_coc5.workspace.jumpTo(location.uri);
  };
}
function viewXir(ctx, xir) {
  return async () => {
    const { document: document2, position } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document2)) return;
    const param = {
    ...
    const req = xir === "HIR" ? viewHir : viewMir;
    const ret = await ctx.client.sendRequest(req, param);
    if (!ret) return;
    const nvim2 = import_coc5.workspace.nvim;
    nvim2.pauseNotification();
    nvim2.command(`edit +setl\\ buftype=nofile [${xir}]`, true);
    nvim2.command("setl nobuflisted bufhidden=wipe", true);
    nvim2.call("append", [0, ret.split("\n")], true);
    nvim2.command("exe 1", true);
    await nvim2.resumeNotification(true);
...
function viewMir2(ctx) {
  return viewXir(ctx, "MIR");
}
function interpretFunction2(ctx) {
  return async () => {
    const { document: document2, position } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document2)) return;
    const param = {
    ...
    const ret = await ctx.client.sendRequest(interpretFunction, param);
    if (!ret) return;
    const nvim2 = import_coc5.workspace.nvim;
    nvim2.pauseNotification();
    nvim2.command("edit +setl\\ buftype=nofile [interpretFunction]", true);
    nvim2.command("setl nobuflisted bufhidden=wipe", true);
    nvim2.call("append", [0, ret.split("\n")], true);
    nvim2.command("exe 1", true);
    await nvim2.resumeNotification(true);
...
function viewFileText2(ctx) {
  return async () => {
    ...
    const nvim2 = import_coc5.workspace.nvim;
    nvim2.pauseNotification();
    nvim2.command("edit +setl\\ buftype=nofile [TEXT]", true);
    nvim2.command("setl nobuflisted bufhidden=wipe", true);
    nvim2.call("append", [0, ret.split("\n")], true);
    nvim2.command("exe 1", true);
    await nvim2.resumeNotification(true);
...
function echoRunCommandLine(ctx) {
  return async () => {
    ...
    import_coc5.window.echoLines([commandLine]);
  };
}
function peekTests(ctx) {
  return async () => {
    ...
    const locations = tests.map(
      (it) => import_coc5.Location.create(it.runnable.location.targetUri, it.runnable.location.targetSelectionRange)
    );
    await import_coc5.commands.executeCommand("editor.action.showReferences", import_coc5.Uri.parse(document2.uri), position, locations);
  };
}
function moveItem2(ctx, direction) {
  return async () => {
    ...
    const mode = await import_coc5.workspace.nvim.call("visualmode");
    if (mode) range = await import_coc5.window.getSelectedRange(mode);
    if (!range) range = import_coc5.Range.create(position, position);
    const params = {
      direction,
      textDocument: { uri: document2.uri },
      range
    ...
    if (!(edits == null ? void 0 : edits.length)) return;
    const wsEdit = {
...
function moveItemUp(ctx) {
  return moveItem2(ctx, "Up");
}
function moveItemDown(ctx) {
  return moveItem2(ctx, "Down");
}
...
`;
    const tempFile = (0, import_node_path4.join)((0, import_node_os.tmpdir)(), `${(0, import_node_crypto2.randomBytes)(5).toString("hex")}.html`);
    (0, import_node_fs4.writeFileSync)(tempFile, html, { encoding: "utf-8" });
    import_coc5.window.showMessage(`Crate Graph: ${tempFile}`);
    await import_coc5.workspace.nvim.call("coc#ui#open_url", [tempFile]);
  };
}
...
function viewItemTree2(ctx) {
  return async () => {
    ...
    const nvim2 = import_coc5.workspace.nvim;
    nvim2.pauseNotification();
    nvim2.command("edit +setl\\ buftype=nofile [ItemTree]", true);
    nvim2.command("setl nobuflisted bufhidden=wipe", true);
    nvim2.call("append", [0, ret.split("\n")], true);
    nvim2.command("exe 1", true);
    await nvim2.resumeNotification(true);
...
function rebuildProcMacros2(ctx) {
  return async () => {
    const { document: document2 } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document2)) return;
    await ctx.client.sendRequest(rebuildProcMacros);
  };
}
...
async function activate(context) {
  ...
  if (!(0, import_node_fs5.existsSync)(serverRoot)) {
    (0, import_node_fs5.mkdirSync)(serverRoot);
  }
  const bin = ctx.resolveBin();
  if (!bin) {
    ...
    if (ctx.config.prompt === true) {
      ret = await import_coc6.window.showQuickpick(["Yes", "Cancel"], msg);
    }
    if (ret === 0) {
      try {
  ...
  ctx.registerCommand("testCurrent", testCurrent);
  ...
  ctx.registerCommand("runFlycheck", runFlycheck2);
  ctx.registerCommand("cancelFlycheck", cancelFlycheck2);
  ctx.registerCommand("clearFlycheck", clearFlycheck2);
  ctx.registerCommand("analyzerStatus", analyzerStatus2);
  ctx.registerCommand("viewCrateGraph", viewCrateGraph2);
  ctx.registerCommand("interpretFunction", interpretFunction2);
  ctx.registerCommand("rebuildProcMacros", rebuildProcMacros2);
